...
module.exports = {
    createNewJsonSim: function (scenario, sceIndex, projectName, modIndex, processModel) {
        var newJson = {"_declaration": {"_attributes": {"version": "1.0", "encoding": "UTF-8"}}};
        newJson.definitions = createSimConfig(scenario, sceIndex, projectName, modIndex, processModel);
        return newJson;
    }
}

function createSimConfig(scenario, sceIndex, projectName, modIndex, processModel) {
    ...
    // create Gateways:
    gatewayTypes = processModel.modelParameter.gateways.map(value => value.type)
    ...
    for (let i in uniqueGatewayTypes) {
        simConfig[uniqueGatewayTypes[i].charAt(0).toLowerCase() + uniqueGatewayTypes[i].slice(1)] = conv_ele.createGateways(processModel.modelParameter.gateways, uniqueGatewayTypes[i]);
    }
    ...
    for (let k in uniqueGatewayTypes) {
        for (let i in simConfig[uniqueGatewayTypes[k]]) {
            for (let j in simConfig[uniqueGatewayTypes[k]][i].outgoingSequenceFlow) {
                ...
                proba = processModel.modelParameter.sequences.find(s => s.id == myId).probability
    ...
    // create events:
    eventTypes = processModel.modelParameter.events.map(value => value.type)
    ...
    for (let i in uniqueEventTypes) {
        simConfig[uniqueEventTypes[i].charAt(0).toLowerCase() + uniqueEventTypes[i].slice(1)] = conv_ele.createEvents(processModel.modelParameter.events, uniqueEventTypes[i]);
    }
    ...
    attributes.processRef = processModel.BPMN.definitions.process[0].ATTR.id;
